...
function isValidCombined(current){
    return (current.artist + current.name).length < 25
}
const totalAlbumSales = musicData.filter(isValidCombined).reduce((acumulado, current) => {
        return acumulado = acumulado + current.sales
    },0);
